...
public class CatalogFeaturesOption extends AbstractFeaturesOption {
    ...
    public void setOverriden(Class <? extends Feature> featureClass, boolean override) {
        ...
        if (override && !overriden.get(featureClass)) {
            FeaturesOption fo = null;
            
            // Decide from where to take the FeatureOption to fill the new one
            if (Options.AppPrefs.getFeaturesOption().isOverriden(featureClass))
                fo = Options.AppPrefs.getFeaturesOption();
            else
                fo = DefaultPreferences.defaultPreferences.getFeaturesOption();
