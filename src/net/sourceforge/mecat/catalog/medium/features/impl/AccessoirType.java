 ...
 * @author Stephan Richard Palm
 *
 * AccessoirType is declared final because it extends ChoiceFeature and sets the attributeName.
 * All classes extending Format would save there information on the same spot.
 */
public class AccessoirType extends MultiChoiceFeature {

	public AccessoirType(Medium medium) {
        super(medium, AbstractFeature.getEasyClassName(AccessoirType.class.getName()));
	}	

	static public ConfigurableComparator getComparator(){
		return new ByChoiceFromMultiChoiceFeature(AccessoirType.class);
	}
}
